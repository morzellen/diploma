...
class CaptionGenerator(BaseGenerator):
    ...
    UNWANTED_PATTERNS: ClassVar[Tuple[str, ...]] = (".", "gif", "png", "jpg", "jpeg", "[ unused0 ]")
    ...
    def _setup_components(self) -> None:
        ...
        with torch.no_grad():
            ...
            tokenizer = self.model_creator.tokenizer
    ...
    def generate(self, image_path: str, image_name: Optional[str] = None) -> str:
        ...
        try:
            ...
            raise CaptionGenerationError(f"Image processing failed for {image_name}") from e
        finally:
            self.handle_memory()

    def _process_image(self, image_path: str, image_name: str) -> BatchEncoding:
    ...
    def _generate_caption(self, inputs: BatchEncoding) -> torch.Tensor:
        """Генерация описания с использованием класса устройства"""
        with torch.inference_mode(), torch.autocast(self.device):
            # Переносим все элементы inputs на устройство
            inputs = {k: v.to(self.device) for k, v in inputs.items()}
            return self.model.generate(**inputs, **self.generation_params)
    ...
    def _postprocess(self, generated_ids: torch.Tensor, image_name: str) -> str:
        ...
        try:
            ...
            logger.info(f"Generated for {image_name}: {filtered}")
            return filtered
        except Exception as e:
